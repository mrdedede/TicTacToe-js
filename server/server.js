 ...
 */
function newGame(pl1, pl2, roomCode) {
 ...
 * @param {string} playerName - The player's name 
 */
function checkWaitlist(playerName) {
  let players = JSON.parse(fs.readFileSync('players.json'))
  let gameCode = null
  let newWaitlist = []
...
app.post('/waitlist/join/:player', function(req, res) {
  return wait(req.params.player)
})

app.post('/waitlist/check/:player', function(req, res) {
  return checkWaitlist(req.params.player)
})

app.post('/game/play/:code/:player/:x/:y', function(req, res) {
  return play(req.params.x, req.params.y, req.params.player, req.params.code)
})

app.delete('/game/delete/:code', function(req, res) {
  deleteGame(req.params.code)
})
